...
import com.qfedu.fmmal.utils.PageHelper;
...
//        List<ProductCommentsVO> productCommentsVOS = productCommentsMapper.selecetCommentsByProductId(productId);
        ...
        // 2.计算总页数（必须确定）每页显示多少页 pageSize = limit
        int pageCount = (count % limit == 0) ? count / limit : count / limit + 1;
        int start = (pageNum - 1) * pageCount;

        // 3.查询当前页的数据（因为评论中需要用户信息，因此需要：【自定义】连表查询--自定义）
        List<ProductCommentsVO> list = productCommentsMapper.selecetCommentsByProductId(productId, start, limit);
